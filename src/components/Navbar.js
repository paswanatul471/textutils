    ...
    <nav className={`navbar navbar-expand-lg navbar-${props.mode} bg-${props.mode}`}>
        ...
        <div className="collapse navbar-collapse" id="navbarSupportedContent">
          ...
          <div className="d-flex">
            ...
            <div className="bg-light rounded mx-1" onClick={ ()=>{props.toggleMode('light') }}style={{height:'25px', width:'25px', cursor:'pointer', border: '2px solid black'}}></div>
            <div className="bg-dark rounded mx-1" onClick={ ()=>{props.toggleMode('dark') }}style={{height:'25px', width:'25px', cursor:'pointer', border: '2px solid white'}}></div>
          </div>
          {/* <div className={`form-check form-switch text-${props.mode === 'light' ? 'grey' : 'white'}`}>
            <input className="form-check-input" onClick={()=>{props.toggleMode(null)}} type="checkbox" role="switch" id="flexSwitchCheckDefault"/>
            <label className="form-check-label" htmlFor="flexSwitchCheckDefault">Toggle kMode</label>
          </div> */}
        </div>
